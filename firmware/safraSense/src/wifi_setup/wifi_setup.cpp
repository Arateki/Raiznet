...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const resetAdvancedFields = () => {
    ['name', 'ext_name', 'ext_url', 'loc_url'].forEach(name => {
      const input = document.querySelector('[name="' + name + '"]');
      if (input && input.hasAttribute('data-default-value')) input.value = input.getAttribute('data-default-value');
    });
  };
  ...
  const setupAdvancedSection = async (lang) => {
    ...
    const tightenSaveButton = () => {
      ...
      if (submit) removeBreaksBefore(submit);
    };

    const moveAdvancedField = (labelKey, inputName) => {
      const input = document.querySelector('[name="' + inputName + '"]');
      if (!input || input.closest('#advanced-body')) return;
      input.setAttribute('data-default-value', input.value || '');
      removeBreaksBefore(input);

      const label = document.querySelector('label[data-i18n="' + labelKey + '"]');
      const field = document.createElement('div');
      field.className = 'advanced-field';
      if (label) field.appendChild(label);
      field.appendChild(input);
      (serverFields || body).appendChild(field);
    };

    [
      ['sensor_name', 'name'],
      ['pub_server_name', 'ext_name'],
      ['ext_server', 'ext_url'],
      ['loc_server', 'loc_url']
    ].forEach(pair => moveAdvancedField(pair[0], pair[1]));
    tightenSaveButton();
...
void setupWifi(DeviceConfig& cfg) {
  ...
  wm.setWebServerCallback([&wm, &id]() {
    wm.server->on("/portal.js", HTTP_GET, [&wm]() {
      wm.server->send(200, "application/javascript", PORTAL_SCRIPT);
    });
  ...
  if (wm.server && wm.server->args() > 0) {
    ...
    if (connectRaiznet && String(p_ext_name.getValue()).length() > 0) {
      cfg.servers_external.push_back({ String(p_ext_name.getValue()), String(p_ext_url.getValue()) });
    }
    if (connectRaiznet && String(p_loc_url.getValue()).length() > 0) {
