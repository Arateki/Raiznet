...
::: info Update (2026-06)
The signed event log is not implemented yet — today ingest writes directly to SQLite (the "Phase 1" path described under Trade-offs). The log design has also moved away from Hypercore toward a Raiznet-native append-only event log ([ADR-004](/en/adr/004-raiznet-native-replication)); the SQLite-as-derived-index principle is unchanged.
:::
