...
The source of truth becomes an append-only event log per author, hash-chained, with every event signed (Ed25519). SQLite remains the derived index ([ADR-002](/en/adr/002-sqlite-cache)); the canonical binary encoding remains Protobuf when it lands ([ADR-001](/en/adr/001-protobuf)). Hypercore is not used, ported, or emulated.
