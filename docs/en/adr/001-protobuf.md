...
::: info Update (2026-06)
The wire format in production is **JSON** with a [signed raw string](/en/protocol/telemetry#the-signed-raw-string); the `.proto` schemas exist but code generation is not active. Protobuf remains the target canonical format and will land together with the event log — JSON stays supported for the current firmware generation.
:::
