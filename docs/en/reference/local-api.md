...
The local endpoint exposes the **same routes** as the [Public API](/en/reference/public-api) with one structural difference: which database they touch.
...
To read local data from outside the LAN, use a private overlay network (Tailscale, WireGuard) to reach `127.0.0.1:3001` on the server — or mark fields as `encrypted` for the public network and decrypt them in your app (see [Privacy Model](/en/protocol/privacy)).
