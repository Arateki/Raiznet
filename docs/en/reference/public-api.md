...
This page documents the API **as implemented today**. The wire format is JSON; a canonical Protobuf encoding is planned (see [Roadmap](/en/guide/roadmap)).
...
Sensor fields are optional. Each one is either `{ "plain": <number> }` or `{ "cipher": "<hex>", "nonce": "<hex>" }`. The signature is Ed25519 (detached) over the bytes of the `raw` string — see [Telemetry](/en/protocol/telemetry) for how `raw` is built. The server verifies it against the **registered** device pubkey, not the one in the payload.
