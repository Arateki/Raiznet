...
These schemas define the **planned** canonical binary encoding ([ADR-001](/en/adr/001-protobuf)). The wire format in production today is JSON with a signed raw string — see [Telemetry](/en/protocol/telemetry). Code generation is not active yet.
:::
...
In the current JSON wire format the signature covers the [pipe-delimited raw string](/en/protocol/telemetry#the-signed-raw-string), not a Protobuf encoding. The canonical-bytes rule for the binary format will be specified when codegen is activated.
:::
