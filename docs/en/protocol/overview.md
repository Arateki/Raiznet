...
Planned additions: a canonical Protobuf encoding ([ADR-001](/en/adr/001-protobuf)), ESP-NOW device-to-device mesh, and server-to-server replication of a signed event log ([ADR-004](/en/adr/004-raiznet-native-replication) — see the [Roadmap](/en/guide/roadmap)).
...
The device signs the UTF-8 bytes of this string with its Ed25519 key and ships both the hex-encoded `raw` and the `signature` inside the JSON block. The server verifies the signature against the device's **registered** pubkey. See [Telemetry](/en/protocol/telemetry) for the full grammar.

Protobuf schemas for a canonical binary encoding already exist in `packages/protocol/proto/` but are not used on the wire yet — see [Proto Schemas](/en/reference/proto-schemas).
...
| Server → Server | Signed event log replication | In design ([ADR-004](/en/adr/004-raiznet-native-replication)) |
