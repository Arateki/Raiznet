...
5. **Insert** with `INSERT OR IGNORE` keyed by `(device_pubkey, seq)`, with `received_at` set to the server clock. Whether the row goes to the public or private database depends on the endpoint and the device's `publishTo` — see [Local API](/en/reference/local-api).
...
The Protobuf schemas in [Proto Schemas](/en/reference/proto-schemas) define the planned canonical encoding for events and telemetry. JSON will remain supported for the current firmware generation and debugging.
