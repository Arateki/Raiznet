...
2. The owner identity is generated or imported in the device's captive portal as a BIP-39 mnemonic — see [Device Lifecycle](/en/protocol/device-lifecycle).
...
The local endpoint (`127.0.0.1:LOCAL_PORT`) will require the owner to prove possession of their User private key (not implemented yet — see [Local API](/en/reference/local-api)):
