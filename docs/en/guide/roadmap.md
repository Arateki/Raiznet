...
- **Signed telemetry ingestion** — Ed25519 signature over a deterministic [raw string](/en/protocol/telemetry#the-signed-raw-string), verified against the registered device key; idempotent batched ingest.
- **Device registry** — `POST /v1/devices` with lazy registration from the firmware.
- **Per-field privacy** — `plain` / `encrypted` / `omit` dispositions with per-destination overrides, enforced at ingestion ([Privacy Model](/en/protocol/privacy)).
...
- **Signed event log** — append-only, hash-chained log per node as the source of truth, with SQLite rebuilt from it as a derived index ([ADR-002](/en/adr/002-sqlite-cache)).
- **Node-to-node replication** ([ADR-004](/en/adr/004-raiznet-native-replication)) — sync v1: HTTP pull between configured peers (LAN, VPN, public IP); sync v2: dial-by-pubkey transport built on an existing Rust P2P foundation (iroh as primary candidate, validated on real rural 4G/CGNAT links before adoption), with community-runnable relays — never a privileged gateway.
- **Networks, filters and catalogs** — topics, `NetworkManifest`, composable MAC filters, `CropCatalog` ([Networks & Filters](/en/protocol/networks)).
- **Local endpoint authentication** — owner challenge-response with the User key ([Local API](/en/reference/local-api)).
- **Combined owner view** — merging public + private readings by `(device_pubkey, seq)` on the local endpoint.
- **Canonical Protobuf encoding** — binary wire format from the schemas in [Proto Schemas](/en/reference/proto-schemas); JSON stays for compatibility ([ADR-001](/en/adr/001-protobuf)).
...
- **DeviceClaim / DeviceTransfer** — ownership chain events ([Device Lifecycle](/en/protocol/device-lifecycle)).
- **Desktop app (Tauri)** bundling a full node, and a mobile app.
- **Intelligence layer** — MCP server over the local endpoint, regional aggregations, collective crop calibration ([Collective Intelligence](/en/guide/intelligence)).

## Compatibility policy

The contracts documented in the [Public API](/en/reference/public-api) and [Telemetry](/en/protocol/telemetry) pages are treated as frozen for the current firmware generation: changes that would break a device in the field (status codes, duplicate semantics, the raw string grammar) are made only behind explicit versioning.
